...
exports.HARNESS_SEED = void 0;
exports.buildCorpusEntries = buildCorpusEntries;
...
exports.HARNESS_SEED = 'phase-4-parity-2026-05-14';
const codexCandidatesJson = (token) => [
    '```json',
    JSON.stringify({
...
const claudeCodeEnvelope = (resultPayload, totalCostUsd) => JSON.stringify({
    type: 'result',
    subtype: 'success',
    is_error: false,
...
const setupImportGraphScope = (ws) => {
...
function buildCorpusEntries() {
    return [
        ...
        },
        {
            ...
            setupWorkspace: (ws) => fs.writeFileSync(path.join(ws, 'tainted.txt'), 'FORBIDDEN_TOKEN_CORPUS_C', 'utf8'),
            stdout: codexInnocentJson(),
            stderr: 'model: o4-mini',
            exitCode: 0,
        },
        ...
        },
        {
            ...
            setupWorkspace: (ws) => {
                ...
                fs.writeFileSync(path.join(srcDir, 'widget.ts'), 'export function compute(x: number): number {\n  return x;\n}\n', 'utf8');
            },
...
function costTokens(dollarsApiEquivalent, fallback) {
...
async function runEntry(entry) {
    ...
    try {
        ...
        switch (entry.adapter) {
            case 'codex':
                outcome = await new adapters_1.CliFalsifier(adapters_1.codexProfile, {
                    authMethodOverride: () => 'api',
                    invocationOverride: invocation,
                }).falsify(input);
                break;
            case 'copilot':
                outcome = await new adapters_1.CliFalsifier(adapters_1.copilotProfile, {
                    authMethodOverride: () => 'chatgpt',
                    invocationOverride: invocation,
                }).falsify(input);
                break;
            case 'claude-code':
                outcome = await new adapters_1.CliFalsifier(adapters_1.claudeCodeProfile, {
                    authMethodOverride: () => 'api',
                    invocationOverride: invocation,
                }).falsify(input);
                break;
        }
        const verdict = outcome.result.kind === 'no-falsification-found'
...
async function runCorpus() {
    const rows = [];
    for (const entry of buildCorpusEntries())
...
async function main() {
    ...
    assert_1.strict.ok(rows.length > 0, 'corpus must emit at least one row');
}
