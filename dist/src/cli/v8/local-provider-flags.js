...
function buildLocalProviderFlagValues(values, resolveModulePath) {
    ...
    if (grammar !== null) {
        if (!exports.LOCAL_GRAMMAR_MODES.includes(grammar)) {
            throw new Error(`invalid --local-grammar "${grammar}"; expected one of: ${exports.LOCAL_GRAMMAR_MODES.join(', ')}`);
        }
        out.grammar = grammar;
    }
...
function resolveEffectiveLocalProvider(fromFlag, fromConfig, env = process.env) {
    return {
        ...
        grammar: fromFlag.grammar ??
            (env['LOCAL_LLM_GRAMMAR'] && exports.LOCAL_GRAMMAR_MODES.includes(env['LOCAL_LLM_GRAMMAR'])
