...
function parseFlags(argv) {
    ...
    return {
        ...
        fix: (0, argv_schema_1.readBoolean)(values, 'fix') ?? false,
        connectors: (0, argv_schema_1.readBoolean)(values, 'connectors') ?? false,
        helpRequested,
    };
}
...
function probeConnectorSurface(cwd) {
    const out = [];
    ...
    try {
        const det = require('../../audit/cheat-detector');
    ...
    try {
        ...
        aibomOk = false;
        aibomDetail = `cannot write AI-BOM artifacts to ${aibomDir}: ${err.message}`;
    }
    out.push({
 ...
 */
function probeSwarmDirectory(cwd) {
    const results = [];
...
const DEFAULT_CONTRACT = ['obligations:', ...DEFAULT_OBLIGATIONS.flatMap((o) => [`  - type: ${o.type}`, `    command: ${o.command}`])].join('\n') +
    '\n';
function defaultPatchEnvelopeCount(obligationType) {
...
const DEFAULT_PATCHES = DEFAULT_OBLIGATIONS.flatMap((o) => Array.from({ length: defaultPatchEnvelopeCount(o.type) }, () => DEFAULT_PATCH_LINE)).join('\n') + '\n';
/**
 * Attempt to auto-fix a probe result. Returns true if the fix was applied.
 */
function applyFix(r, cwd) {
    const swarmDir = path.join(cwd, '.swarm');
    try {
        ...
        logger.warn(`  FIX FAILED: could not auto-fix "${r.name}": ${err.message}`);
        return false;
    }
    // Not fixable by us
    return false;
