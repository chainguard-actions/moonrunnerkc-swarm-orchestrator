...
exports.normalizeLeadingGlobalFlags = normalizeLeadingGlobalFlags;
exports.parseSwarmFlags = parseSwarmFlags;
function parseOutputFormat(args) {
    if (args.includes('--json'))
        return 'json';
    const outputIndex = args.indexOf('--output');
    if (outputIndex !== -1 && args[outputIndex + 1] === 'json')
        return 'json';
    return 'text';
}
function extractPositionalArgs(args, options = {}) {
...
function normalizeLeadingGlobalFlags(args) {
...
function parseSwarmFlags(args) {
    ...
    if (modelIndex !== -1 && args[modelIndex + 1])
        opts.model = args[modelIndex + 1];
    if (args.includes('--confirm-deploy'))
        opts.confirmDeploy = true;
    if (args.includes('--no-quality-gates'))
        opts.noQualityGates = true;
    if (args.includes('--pm'))
        opts.pm = true;
    if (args.includes('--strict-isolation'))
        opts.strictIsolation = true;
    if (args.includes('--lean'))
        opts.lean = true;
    if (args.includes('--verbose'))
        opts.verbose = true;
    if (args.includes('--useInnerFleet') || args.includes('--wrap-fleet'))
        opts.useInnerFleet = true;
    if (args.includes('--cost-estimate-only'))
        opts.costEstimateOnly = true;
    if (args.includes('--yes') || args.includes('-y'))
        opts.yes = true;
    if (args.includes('--quiet') || args.includes('-q'))
        opts.quiet = true;
    if (args.includes('--stream-agent'))
        opts.streamAgent = true;
    ...
    if (maxPremIdx !== -1 && args[maxPremIdx + 1]) {
        ...
        if (isNaN(parsed) || parsed < 0) {
            throw new Error(`--max-premium-requests requires a non-negative integer, got "${args[maxPremIdx + 1]}"`);
        }
    ...
    if (maxRetriesIdx !== -1 && args[maxRetriesIdx + 1]) {
        ...
        if (isNaN(parsed) || parsed < 0) {
            throw new Error(`--max-retries requires a non-negative integer, got "${args[maxRetriesIdx + 1]}"`);
        }
        opts.maxRetries = parsed;
    }
    const resumeIndex = args.indexOf('--resume');
    if (resumeIndex !== -1 && args[resumeIndex + 1])
        opts.session = args[resumeIndex + 1];
    ...
    if (prIndex !== -1 && args[prIndex + 1]) {
        ...
        if (mode !== 'auto' && mode !== 'review') {
            throw new Error(`--pr requires "auto" or "review", got "${mode}"`);
        }
        opts.prMode = mode;
    }
    ...
    if (args.includes('--owasp-report'))
        opts.owaspReport = true;
