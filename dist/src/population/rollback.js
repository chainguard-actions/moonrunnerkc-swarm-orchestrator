...
exports.rollbackObligation = rollbackObligation;
 ...
 */
function findLatestSnapshot(ledger, obligationIndex) {
 ...
 */
async function rollbackObligation(obligationIndex, ledger, repoRoot, runId, _trigger) {
    ...
    for (const f of snapshot.files) {
        ...
        let currentSha;
        try {
            currentSha = fs.existsSync(absPath) ? (0, diff_snapshot_1.gitHashObject)(fs.readFileSync(absPath)) : 'absent';
        }
        catch (err) {
            return {
                success: false,
                ...
                failure: {
                    kind: 'io-error',
                    detail: `failed to read current state of ${f.path}: ${err.message}`,
                },
            };
        }
        ...
        try {
            if (f.preBlobSha === 'absent') {
                ...
                const afterSha = (0, diff_snapshot_1.gitHashObject)(fs.readFileSync(absPath));
            ...
            return {
                ...
                failure: {
                    kind: 'io-error',
                    detail: `rollback for obligation ${obligationIndex} failed: ${err.message}`,
                },
            };
        }
    }
