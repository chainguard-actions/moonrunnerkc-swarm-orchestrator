...
exports.renderDynamicMessage = renderDynamicMessage;
...
// dominate.
function renderDynamicMessage(obligation, repoRoot, context) {
    const lines = [
        `Obligation:`,
    ...
    switch (obligation.type) {
        case 'file-must-exist': {
            ...
            lines.push('Wrap the file body in a single fenced code block. No prose outside the fences.');
            break;
        }
        case 'build-must-pass':
            ...
            lines.push('Otherwise output a unified diff against repo root that makes the build pass.');
            lines.push('Use repo-relative paths in diff headers (`--- a/path` and `+++ b/path`); never write outside existing files unless the diff explicitly creates a new path the obligation requires.');
        ...
        case 'test-must-pass':
            ...
            lines.push('Use repo-relative paths in diff headers; never write outside existing files unless the diff explicitly creates a path the obligation requires.');
            if (context?.testFramework) {
                lines.push(renderFrameworkPreservationHint(context.testFramework));
            }
            if (context?.commandFailureTail) {
        ...
        case 'function-must-have-signature':
            lines.push(`Function "${obligation.name}" in ${obligation.file} must declare the signature ` +
                `"${obligation.signature}".`);
            lines.push('If the file already declares the function with this signature, output "no-op".');
            lines.push('Otherwise output a unified diff against repo root that brings the file into compliance.');
            (0, file_context_1.appendFileContext)(lines, repoRoot, [obligation.file]);
            break;
        case 'property-must-hold':
            lines.push(`The property over "${obligation.target}" asserted by predicate "${obligation.predicate}" ` +
                `must hold (predicate exits zero).`);
            lines.push('If the property already holds, output "no-op".');
            lines.push('Otherwise output a unified diff against repo root that makes the predicate pass.');
            if (context?.commandFailureTail) {
                lines.push('', renderFailureBlock(obligation.predicate, context.commandFailureTail));
            }
            (0, file_context_1.appendFileContext)(lines, repoRoot, extractFilePathsFromPredicate(obligation.predicate));
            break;
        case 'import-graph-must-satisfy':
            lines.push(`Import graph rooted at ${obligation.scope} must satisfy "${obligation.constraint}".`);
            lines.push('If the constraint already holds, output "no-op".');
            lines.push('Otherwise output a unified diff against repo root that removes the offending edges.');
            break;
        case 'coverage-must-exceed':
            lines.push(`Coverage report ${obligation.scope} must report ${obligation.metric} pct >= ` +
                `${obligation.threshold}%.`);
            lines.push('If coverage already meets the threshold, output "no-op".');
            lines.push('Otherwise output a unified diff against repo root that adds tests until coverage clears the threshold.');
            break;
        case 'performance-must-not-regress':
            lines.push(`Benchmark "${obligation.benchmark}" must not regress past ` +
            ...
            lines.push('Otherwise output a unified diff against repo root that recovers the regression.');
            if (context?.commandFailureTail) {
...
// fs.existsSync check.
function extractFilePathsFromPredicate(predicate) {
    const candidates = [];
...
function renderTestFrameworkHint(relPath, framework) {
    if (!(0, test_framework_misuse_1.isTestFilePath)(relPath))
        return null;
    switch (framework) {
        case 'jest':
...
function renderFrameworkPreservationHint(framework) {
    return (`This project uses the **${framework}** test framework. Preserve it. ` +
...
function renderFailureBlock(command, tail) {
...
function preRunCommandVerifier(obligation, options) {
    ...
    const r = (0, run_verifier_1.verifyObligation)(obligation, options);
    if (r.satisfied)
        return null;
...
function buildRenderContext(obligation, repoRoot, manifest, commandTimeoutMs) {
    const ctx = {};
    if (obligation.type === 'file-must-exist') {
        const fw = manifest.repoContext.testFramework ?? null;
        if (fw !== null)
            ctx.testFramework = fw;
    }
    else {
