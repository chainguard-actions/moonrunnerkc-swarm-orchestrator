...
exports.TransientApiRetryExhaustedError = exports.TRANSIENT_API_ERROR_PATTERN = void 0;
 ...
 */
exports.TRANSIENT_API_ERROR_PATTERN = /Request failed due to a transient API error\.\s*Retrying\.{1,}/i;
/** True when the result looks like a Copilot transient-API-error exit. */
function isTransientApiError(result) {
    if (result.exitCode === 0)
        return false;
    return exports.TRANSIENT_API_ERROR_PATTERN.test(`${result.stdout}\n${result.stderr}`);
}
/**
 ...
 */
class TransientApiRetryExhaustedError extends Error {
    maxAttempts;
    lastResult;
    constructor(maxAttempts, lastResult) {
        ...
        super(`copilot exec hit the transient API error marker on all ${maxAttempts} attempts ` +
            ...
            `Re-run when the upstream provider is healthy or raise maxAttempts.`, { cause: { lastResult } });
...
async function invokeWithTransientRetry(invoke, options) {
    ...
    throw new TransientApiRetryExhaustedError(maxAttempts, lastTransient);
}
