...
exports.parseFencedCandidates = parseFencedCandidates;
/** Extract and validate the fenced JSON candidate document from `rawOutput`. */
function parseFencedCandidates(rawOutput, config) {
    ...
    try {
        ...
        throw new Error(`${config.label} output: fenced \`\`\`json\`\`\` block did not parse as JSON. ` +
            `Inspect captured stdout to debug the prompt.`, { cause });
    }
    return validateCandidates(parsed, config);
}
function extractFencedJson(rawOutput, label) {
    const FENCE = '```json';
    const fenceIdx = rawOutput.indexOf(FENCE);
    if (fenceIdx === -1) {
        throw new Error(`${label} output did not contain a fenced \`\`\`json\`\`\` block. Inspect captured stdout.`);
    }
...
function validateCandidates(parsed, config) {
    ...
    if (root.candidates.length !== config.requiredCount) {
        throw new Error(`${config.label} returned ${root.candidates.length} candidates; expected exactly ${config.requiredCount}.`);
    }
    return root.candidates.map((entry, i) => validateCandidate(entry, i, config.label));
}
function validateCandidate(entry, index, label) {
...
function validateFile(entry, candidateName, j, label) {
    if (entry === null || typeof entry !== 'object') {
    ...
    if (relPath.startsWith('/') || relPath.includes('..')) {
        throw new Error(`${label} candidate "${candidateName}" file[${j}] relPath "${relPath}" must be relative and may not contain "..".`);
    }
