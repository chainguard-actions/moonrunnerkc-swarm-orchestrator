...
class ChainTamperedError extends errors_1.SwarmError {
    lineNumber;
    kind;
    constructor(message, lineNumber, kind, remediation) {
...
class HashChainedLedger {
    ...
    append(payload, opts) {
        ...
        const agentField = opts?.aiAgent !== undefined ? { aiAgent: opts.aiAgent } : {};
        const base = {
            ts,
            runId: this.runId,
            seq: this.seq,
...
function verifyChainEntries(entries) {
    ...
    for (let i = 0; i < entries.length; i += 1) {
        ...
        if (typeof e.prevHash !== 'string' || typeof e.entryHash !== 'string') {
            throw new ChainTamperedError(`ledger entry at line ${lineNumber} is missing prevHash/entryHash header fields`, lineNumber, 'malformed-header', 'Try: delete the corrupted ledger file and re-run, or restore from a backup');
        }
        if (e.prevHash !== expectedPrev) {
            throw new ChainTamperedError(`ledger chain broken at line ${lineNumber}: prevHash ${shortHash(e.prevHash)} does not chain from ${shortHash(expectedPrev)}`, lineNumber, 'prev-hash-mismatch', 'Try: delete the corrupted ledger file and re-run, or restore from a backup');
        }
        const recomputed = computeEntryHash(stripEntryHash(e));
        if (recomputed !== e.entryHash) {
            throw new ChainTamperedError(`ledger entry at line ${lineNumber} fails entry-hash check: stored ${shortHash(e.entryHash)} but recomputed ${shortHash(recomputed)}`, lineNumber, 'entry-hash-mismatch', 'Try: delete the corrupted ledger file and re-run, or restore from a backup');
        }
