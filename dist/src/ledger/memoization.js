...
exports.MemoStore = void 0;
exports.obligationKey = obligationKey;
exports.priorSatisfiedIndexes = priorSatisfiedIndexes;
exports.priorFailedIndexes = priorFailedIndexes;
exports.hitFromMemoized = hitFromMemoized;
...
class MemoStore {
    hashesByType = new Map();
    winnerByHash = new Map();
    constructor(entries = []) {
    ...
    rebuild(entries) {
        ...
        const typeByRunIndex = new Map();
        for (const e of entries) {
            if (e.type === 'obligation-attempted') {
                typeByRunIndex.set(`${e.runId}|${e.obligationIndex}`, e.obligationType);
            }
    ...
    ingestWinner(entry, obligationType) {
    ...
    findPriorWinnerByHash(obligation, responseSha256) {
        const set = this.hashesByType.get(obligation.type);
        if (!set || !set.has(responseSha256))
            return null;
    ...
    hashesIndexedCount() {
        let total = 0;
        for (const set of this.hashesByType.values())
            total += set.size;
        return total;
    }
}
exports.MemoStore = MemoStore;
// `excludeRunId` skips the run currently resuming-into so we don't
// loop on our own partial state.
function priorSatisfiedIndexes(entries, contractHash, options = {}) {
    const eligibleRuns = new Set();
    for (const e of entries) {
        if (e.type === 'run-started') {
            const re = e;
            if (re.contractHash === contractHash)
                eligibleRuns.add(re.runId);
        }
    }
    if (options.excludeRunId)
        eligibleRuns.delete(options.excludeRunId);
    const satisfied = new Set();
    const failed = new Set();
    for (const e of entries) {
        if (!eligibleRuns.has(e.runId))
            continue;
...
function priorFailedIndexes(entries, contractHash, options = {}) {
    const eligibleRuns = new Set();
    for (const e of entries) {
        if (e.type === 'run-started') {
            const re = e;
...
function hitFromMemoized(entry) {
    return {
        source: 'prior-run',
        origin: entry,
