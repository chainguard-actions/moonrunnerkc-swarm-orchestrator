...
exports.testRelaxationDetector = void 0;
const diff_walker_1 = require("./diff-walker");
const VERSION = '1.0.0';
const STRICT_MATCHERS = [
    /\btoBe\s*\(/,
    /\btoEqual\s*\(/,
    /\btoStrictEqual\s*\(/,
    /\btoMatch\s*\(/,
...
const TEST_BLOCK_OPENERS = [
    /\bdescribe\s*\(/,
    /\bit\s*\(/,
    /\btest\s*\(/,
    /\bdef\s+test_/,
];
exports.testRelaxationDetector = {
    ...
    run(ctx) {
        ...
        for (const hunk of hunks) {
            ...
            for (const del of deletions) {
                if (!isStrictAssertion(del.content))
                    continue;
                ...
                if (sibling !== undefined) {
                    findings.push({
                        ...
                        severity: 'block',
                        message: 'Existing strict assertion was replaced with a looser matcher. ' +
            ...
            for (const del of hunk.deleted) {
                if (TEST_BLOCK_OPENERS.some((re) => re.test(del.content))) {
                    const replaced = hunk.added.some((a) => TEST_BLOCK_OPENERS.some((re) => re.test(a.content)));
                    if (!replaced) {
                        findings.push({
                            category: 'test-relaxation',
                            severity: 'block',
                            message: 'Test block was removed without a replacement in the same hunk. ' +
            ...
            if (pureDeletions.length > 0 && additions.length === 0) {
                for (const del of pureDeletions) {
                    findings.push({
                        category: 'test-relaxation',
                        severity: 'warn',
                        message: 'Assertion-bearing line was deleted with no compensating assertion in the same hunk.',
                        location: { file: hunk.file, line: del.lineNumber },
                        evidence: `- ${del.content.trim()}`,
                    });
                }
...
function containsAssertion(line) {
    return ASSERTION_LINE.some((re) => re.test(line));
}
function isStrictAssertion(line) {
    return STRICT_MATCHERS.some((re) => re.test(line));
}
function isLooseAssertion(line) {
