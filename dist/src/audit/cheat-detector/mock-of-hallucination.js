...
exports.mockOfHallucinationDetector = void 0;
...
const GO_MOCK_PATTERNS = [
    /mock\.Register\(\s*"([^"]+)"/,
];
exports.mockOfHallucinationDetector = {
        ...
        for (const hunk of hunks) {
            for (const addition of hunk.added) {
                ...
                if (claimed === undefined)
                    continue;
                if (isLocalImport(claimed))
                    continue;
                const root = topLevelPackageOf(claimed);
                if (knownDeps.has(root))
                    continue;
                if (root.length === 0)
                    continue;
                findings.push({
                    ...
                    severity: 'block',
                    message: `Mocked module "${claimed}" is not declared in any project manifest ` +
...
function extractMockTarget(line) {
...
function topLevelPackageOf(target) {
...
function collectKnownDependencies(repoRoot) {
    const out = new Set();
...
function readPackageJson(repoRoot, out) {
...
function readRequirementsTxt(repoRoot, out) {
...
function readPyprojectToml(repoRoot, out) {
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (m?.[1] !== undefined && m[1].toLowerCase() !== 'python')
            out.add(m[1]);
        depRe.lastIndex = 0;
...
function readGoMod(repoRoot, out) {
...
function readCargoToml(repoRoot, out) {
...
function parseJsonOrEmpty(text) {
    try {
        ...
        throw new Error(`mock-of-hallucination: failed to parse package.json: ${err.message}`, { cause: err });
    }
}
