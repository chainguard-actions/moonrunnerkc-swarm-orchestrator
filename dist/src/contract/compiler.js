...
const predicate_runner_1 = require("../shared-predicates/predicate-runner");
...
class ContractValidationError extends errors_1.SwarmError {
    obligations;
    validationErrors;
    constructor(obligations, errors, remediation) {
...
async function compileGoal(options) {
    ...
    if (!validation.valid) {
        throw new ContractValidationError(extracted.obligations, validation.errors, 'Try: check your contract.yaml syntax against the schema in docs/');
    }
    ...
    const tagged = autoTag
        ? (0, tagger_1.tagObligations)(filteredObligations, {
            availableStrategies: options.availableStrategies ?? strategy_constants_1.DEFAULT_STRATEGY_NAMES,
        })
...
function finalize(draft, now = new Date()) {
    ...
    if (!validation.valid) {
        throw new ContractValidationError(draft.obligations, validation.errors, 'Try: check your contract.yaml syntax against the schema in docs/');
    }
