 ...
 */
class DeterministicExtractorError extends errors_1.SwarmError {
    issues;
    constructor(issues, summary, remediation) {
...
function loadEnvelopeFile(filePath) {
    ...
    if (!fs.existsSync(absolute)) {
        throw fileError(`contract file not found: ${absolute}; check the --contract-file path`, 'Try: create one with swarm init, or specify --contract-file <path>');
    }
    ...
    throw fileError(`contract file ${absolute} has unsupported extension "${ext}"; use .json, .yaml, or .yml`, 'Try: rename your contract file with a .json, .yaml, or .yml extension');
}
function fileError(message, remediation) {
...
function tryParse(parse, absolute, from, fallback, fallbackExt) {
    try {
        ...
        throw fileError(`contract file ${absolute} is not valid ${from}: ${err.message}; ` +
            `fix the ${from} syntax or use a ${fallbackExt} extension to parse as ${fallback}`, `Try: fix the ${from} syntax in ${absolute}, or rename to ${fallbackExt} to parse as ${fallback}`);
    }
}
async function loadEnvelopeModule(modulePath) {
    const absolute = path.resolve(modulePath);
    if (!fs.existsSync(absolute)) {
        throw fileError(`contract module not found: ${absolute}; check the --contract-module path`, 'Try: verify the module path exists, or use --contract-file instead');
    }
    let mod;
    try {
        ...
        throw fileError(`failed to import contract module ${absolute}: ${err.message}; ` +
            `the module must be a TS/JS file the runtime can load`, 'Try: check the module for syntax errors or missing dependencies, or use --contract-file instead');
    }
...
function validateContractEnvelope(raw, sourceLabel) {
    ...
    if (!validator(raw)) {
        ...
        throw new DeterministicExtractorError(issues, summary, 'Try: check your contract.yaml syntax against the schema in docs/');
    }
    return raw;
}
let cachedValidator;
