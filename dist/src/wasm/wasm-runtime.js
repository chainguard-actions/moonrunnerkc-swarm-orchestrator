...
class SandboxEscapeError extends errors_1.SwarmError {
    /** The path the strategy attempted to write. */
    attemptedPath;
    /** The repoRoot the sandbox is anchored to. */
    repoRoot;
    constructor(attemptedPath, repoRoot, remediation) {
        super(`sandbox refused write at ${attemptedPath}: path escapes repoRoot ${repoRoot}`, 'SANDBOX_ESCAPE', remediation !== undefined ? { remediation } : undefined);
...
class StrategyTimeoutError extends errors_1.SwarmError {
    strategyName;
    timeoutMs;
    constructor(strategyName, timeoutMs, remediation) {
...
function ensureInsideRepoRoot(repoRoot, candidate) {
    ...
    if (rel === '' || rel.startsWith('..') || path.isAbsolute(rel)) {
        if (rel === '')
            return absRepoRoot;
        throw new SandboxEscapeError(candidate, absRepoRoot, 'Try: check that your strategy only writes inside the repository root, or use --repo-root to set the correct boundary');
    }
    return resolved;
}
...
async function runWithTimeout(strategy, ctx) {
    ...
    const timeoutPromise = new Promise((_resolve, reject) => {
        timer = setTimeout(() => {
            reject(new StrategyTimeoutError(strategy.name, ctx.timeoutMs, 'Try: increase --command-timeout-ms, or use --preset fast to skip pre-generation checks'));
        }, ctx.timeoutMs);
    });
    try {
