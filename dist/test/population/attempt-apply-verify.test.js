...
const apply_verify_1 = require("../../src/population/apply-verify");
...
const FILE_MUST_EXIST = { type: 'file-must-exist', path: 'OUT.txt' };
const TEST_MUST_PASS = { type: 'test-must-pass', command: 'true' };
const TEST_MUST_FAIL = { type: 'test-must-pass', command: 'false' };
describe('population/manager — attemptApplyAndVerify', () => {
    it('applies a file-must-exist obligation and reports success', async () => {
        ...
        const result = await (0, apply_verify_1.attemptApplyAndVerify)({
            ...
            ledger,
            runId: 'r1',
            fileMustExistPaths: new Set(['OUT.txt']),
        ...
        assert_1.strict.equal(result.applyOk, true);
        assert_1.strict.ok(/wrote OUT\.txt/.test(result.applyDetail));
        assert_1.strict.ok(fs.existsSync(path.join(repo, 'OUT.txt')));
    });
    it('reports applyOk=true for a legitimate no-op against a passing test command', async () => {
        const repo = tmpDir('aaaV-');
        const ledger = new jsonl_ledger_1.JsonlLedger(path.join(repo, 'ledger.jsonl'), 'r2');
        const result = await (0, apply_verify_1.attemptApplyAndVerify)({
        ...
        assert_1.strict.equal(result.applyOk, true);
        assert_1.strict.equal(result.applyDetail, 'no-op declared');
    });
    it('reports applyOk=false and unsatisfied for prose against a failing command', async () => {
        const repo = tmpDir('aaaV-');
        const ledger = new jsonl_ledger_1.JsonlLedger(path.join(repo, 'ledger.jsonl'), 'r3');
        const result = await (0, apply_verify_1.attemptApplyAndVerify)({
        ...
        assert_1.strict.equal(result.applied, false);
        assert_1.strict.equal(result.applyOk, false);
        assert_1.strict.ok(result.applyDetail.startsWith('persona response is neither'));
    });
    it('reports applyOk=false on a unified diff whose context does not match the workspace', async () => {
        const repo = tmpDir('aaaV-');
        const ledger = new jsonl_ledger_1.JsonlLedger(path.join(repo, 'ledger.jsonl'), 'r4');
        ...
        assert_1.strict.equal(result.satisfied, false);
        assert_1.strict.equal(result.applyOk, false);
        assert_1.strict.ok(/unified diff (did not apply|parse\/apply error)/.test(result.applyDetail), `expected diff failure detail, got: ${result.applyDetail}`);
    });
    it('rolls back a failed-apply attempt and emits a rolled-back ledger entry', async () => {
        ...
        const ledger = new jsonl_ledger_1.JsonlLedger(path.join(repo, 'ledger.jsonl'), 'r5');
        ...
        assert_1.strict.equal(restored, 'original-content\n');
        const entries = ledger.readAll();
        const rolled = entries.find((e) => e.type === 'obligation-rolled-back');
        assert_1.strict.ok(rolled, 'expected an obligation-rolled-back ledger entry');
        assert_1.strict.equal(rolled.trigger, 'per-obligation-failed-apply');
    });
    it('does NOT roll back a file-must-exist failure (architect file creation has no pre-state)', async () => {
        ...
        const obligation = { type: 'file-must-exist', path: 'foo.test.ts' };
        const ledger = new jsonl_ledger_1.JsonlLedger(path.join(repo, 'ledger.jsonl'), 'r6');
        const result = await (0, apply_verify_1.attemptApplyAndVerify)({
            obligation,
            obligationIndex: 0,
            responseText: fenced,
            ...
            ledger,
            runId: 'r6',
            fileMustExistPaths: new Set(['foo.test.ts']),
            commandTimeoutMs: undefined,
        ...
        const entries = ledger.readAll();
        const rolled = entries.find((e) => e.type === 'obligation-rolled-back');
        assert_1.strict.equal(rolled, undefined, 'file-must-exist must not roll back');
    });
    it('passes through the test-framework-misuse override when the architect lands the wrong framework', async () => {
        ...
        const fenced = '```\nimport { describe } from \'@jest/globals\';\nexpect(1).toBe(1);\n```';
        const obligation = { type: 'file-must-exist', path: 'foo.test.ts' };
        const ledger = new jsonl_ledger_1.JsonlLedger(path.join(repo, 'ledger.jsonl'), 'r7');
        const result = await (0, apply_verify_1.attemptApplyAndVerify)({
            obligation,
            obligationIndex: 0,
            responseText: fenced,
        ...
        assert_1.strict.equal(result.satisfied, false);
        assert_1.strict.ok(/wrong test framework/.test(result.verifyDetail), `expected test-framework-misuse detail, got: ${result.verifyDetail}`);
    });
});
