...
const codex_2 = require("../../../../src/falsification/adapters/profiles/codex");
/**
...
function validCandidatesObject() {
    return {
        candidates: Array.from({ length: codex_2.CODEX_CANDIDATE_COUNT }, (_, i) => validCandidate(`candidate-${i}`)),
    };
}
describe('parseCodexCandidates', () => {
    it('parses a well-formed fenced JSON document', () => {
        const json = JSON.stringify(validCandidatesObject());
        const parsed = (0, codex_1.parseCodexCandidates)(fenceJson(json));
        assert_1.strict.equal(parsed.length, codex_2.CODEX_CANDIDATE_COUNT);
        assert_1.strict.equal(parsed[0]?.name, 'candidate-0');
        assert_1.strict.equal(parsed[0]?.files[0]?.relPath, 'candidate-0/leak.txt');
    });
    it('throws when the fenced block is missing', () => {
        assert_1.strict.throws(() => (0, codex_1.parseCodexCandidates)('no fence here at all'), /fenced ```json``` block/);
    });
    it('throws on JSON parse failure (malformed but brace-balanced body)', () => {
        ...
        // braces case below.
        assert_1.strict.throws(() => (0, codex_1.parseCodexCandidates)(fenceJson('{not-a-quoted-key: 1}')), /did not parse as JSON/);
    });
    it('throws on unbalanced braces inside the fence', () => {
        assert_1.strict.throws(() => (0, codex_1.parseCodexCandidates)(fenceJson('{ "candidates": [')), /unbalanced braces/);
    });
    it('throws when the candidate count is wrong', () => {
        const tooFew = { candidates: [validCandidate('only-one')] };
        assert_1.strict.throws(() => (0, codex_1.parseCodexCandidates)(fenceJson(JSON.stringify(tooFew))), /Codex returned 1 candidates/);
    });
    it('throws when a candidate uses an absolute path', () => {
        const bad = validCandidatesObject();
        bad.candidates[0].files[0].relPath =
            '/etc/passwd';
        assert_1.strict.throws(() => (0, codex_1.parseCodexCandidates)(fenceJson(JSON.stringify(bad))), /must be relative/);
    });
    it('throws when a candidate uses ".." escape', () => {
        const bad = validCandidatesObject();
        bad.candidates[0].files[0].relPath =
            '../escape.txt';
        assert_1.strict.throws(() => (0, codex_1.parseCodexCandidates)(fenceJson(JSON.stringify(bad))), /may not contain "\.\."/);
    });
    it('throws on a candidate with empty files array', () => {
        const bad = validCandidatesObject();
        bad.candidates[0].files = [];
        assert_1.strict.throws(() => (0, codex_1.parseCodexCandidates)(fenceJson(JSON.stringify(bad))), /non-empty files array/);
    });
    it('accepts a candidate file with empty bytes (legitimate empty-file payload)', () => {
        const ok = validCandidatesObject();
        ok.candidates[0].files = [
            { relPath: '.env', bytes: '' },
        ];
        const parsed = (0, codex_1.parseCodexCandidates)(fenceJson(JSON.stringify(ok)));
        assert_1.strict.equal(parsed[0]?.files[0]?.relPath, '.env');
    ...
    it('survives JSON whose string content contains nested triple-backticks', () => {
        ...
        const parsed = (0, codex_1.parseCodexCandidates)(wrapped);
        assert_1.strict.equal(parsed.length, codex_2.CODEX_CANDIDATE_COUNT);
        assert_1.strict.equal(parsed[0]?.files[0]?.bytes.includes('```text'), true);
    });
