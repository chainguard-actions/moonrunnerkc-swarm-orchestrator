...
describe('phase-3 gate fixture contamination guard', () => {
    it('locates the Phase 3 obligation file', () => {
        assert_1.strict.equal(fs.existsSync(PHASE3_SAMPLE_PATH), true, `missing phase-3 obligations file: ${PHASE3_SAMPLE_PATH}`);
    });
    it('every Phase 3 obligation is satisfied against a fresh copy of the referenced fixture', () => {
        const sample = JSON.parse(fs.readFileSync(PHASE3_SAMPLE_PATH, 'utf8'));
        assert_1.strict.ok(Array.isArray(sample.obligations) && sample.obligations.length === 20);
        const fixtureRoot = path.resolve(REPO_ROOT, sample.fixturePath);
        assert_1.strict.equal(fs.existsSync(fixtureRoot), true, `phase-3 obligations.json points at missing fixture: ${fixtureRoot}`);
        ...
        try {
            ...
            for (const sampleObligation of sample.obligations) {
                const verdict = (0, run_verifier_1.verifyObligation)(sampleObligation, {
                    repoRoot: workspace,
                });
            ...
            assert_1.strict.deepEqual(failures, [], `phase-3 fixture is contaminated; obligations that did not verify clean:\n${failures.join('\n')}`);
        }
        finally {
