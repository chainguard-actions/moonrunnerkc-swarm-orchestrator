...
describe('cli/v8/run-wrapper splitArgv', () => {
    ...
    it('returns null fields for absent flags and routes unknown flags through to runPassthrough', () => {
        ...
        assert_1.strict.deepEqual(split.compilePassthrough, []);
    ...
    it('rejects --local-* flag without a value', () => {
        assert_1.strict.throws(() => splitArgv(['--local-backend']), /requires a value/);
    });
...
describe('cli/v8/run-wrapper handleRunV8', () => {
    ...
    it('forwards --local-* flags into the compile-handler argv', async () => {
        ...
        assert_1.strict.ok(compileArgsCaptured.includes('--local-backend'));
        assert_1.strict.ok(compileArgsCaptured.includes('ollama'));
        assert_1.strict.ok(compileArgsCaptured.includes('--local-base-url'));
        assert_1.strict.ok(compileArgsCaptured.includes('http://localhost:11434/v1'));
        assert_1.strict.ok(compileArgsCaptured.includes('--local-model-extractor'));
        assert_1.strict.ok(compileArgsCaptured.includes('qwen2.5-coder:14b'));
    });
});
