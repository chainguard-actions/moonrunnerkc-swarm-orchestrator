...
function renderHunk(file, lines) {
...
function renderMultiHunk(parts) {
...
function buildTestRelaxation(i) {
    const fn = `compute${i}`;
...
function buildMockOfHallucination(i) {
...
function buildAssertionStrip(i) {
    const file = `src/feature-${i}.test.ts`;
    const v = `v${i}`;
    return {
        brokenDiff: renderHunk(file, [
...
function buildCoverageErosion(i) {
...
function buildCommentOnlyFix(i) {
...
function buildExceptionRethrowLostContext(i) {
...
function buildDeadBranchInsertion(i) {
...
function main() {
    ...
    fs.writeFileSync(path.join(outRoot, 'package.json'), JSON.stringify(corpusPackageJson, null, 2) + '\n');
