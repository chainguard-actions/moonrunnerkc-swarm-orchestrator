...
const PHASE_7_PERSONA_FOR_TYPE = {
...
async function runHappyPath() {
    ...
    try {
        ...
        const ledger = new jsonl_ledger_1.JsonlLedger(path.join(repoRoot, 'ledger.jsonl'), 'phase7-pass');
        ...
        return {
            personaCount: persona_registry_1.DEFAULT_PERSONA_IDS.length,
            obligationTypeCount: types_1.OBLIGATION_TYPES.length,
            obligations: rows,
...
async function runFailureSuite() {
    ...
    try {
        ...
        const ledger = new jsonl_ledger_1.JsonlLedger(path.join(repoRoot, 'ledger.jsonl'), 'phase7-fail');
        const result = await (0, manager_1.runPopulation)({
            contract,
            repoRoot,
            registry: (0, persona_registry_1.createDefaultRegistry)(),
...
function evaluateGates(happy, failure) {
    const personaOk = happy.personaCount >= 7;
    const typeOk = happy.obligationTypeCount >= 8;
    const dispatchOk = happy.obligations.every((r) => r.satisfied && r.observedPersona === r.expectedPersona);
    ...
    const failedTypes = new Set(failure.obligations.filter((r) => !r.satisfied).map((r) => r.obligationType));
...
function renderReport(args) {
    ...
    lines.push('');
    lines.push('Phase 7 (impl guide §10) is open-ended; the milestone gate for v8.0 release is "at least 7 personas in the library and at least 8 contract obligation types." This benchmark asserts both gates, drives every Phase 7 obligation type end-to-end through the population manager against a `StubSession`, and confirms each obligation type dispatches to its owning persona.');
    lines.push('');
    lines.push('Two suites run back-to-back. The **happy-path suite** plants every workspace artifact the verifier will inspect (a marker README, an `src/api.ts` declaring the expected signature, an acyclic `src/` graph, a passing coverage report, and a baseline benchmark file) so each persona\'s "no-op" reply is enough for verification to clear. The **failure suite** plants a cycle, an incorrect signature, a `false` predicate, a missing coverage file, and a missing baseline; every Phase 7 verifier should report a verifiable failure detail.');
    lines.push('');
    ...
    for (const r of happy.obligations) {
        lines.push(`| ${r.obligationType} | ${r.expectedPersona} | ${r.observedPersona ?? '∅'} | ${r.satisfied ? '✓' : '✗'} | ${truncate(r.detail, 80)} |`);
    }
    lines.push('');
    lines.push(`Run aggregate: ${happy.satisfied} satisfied / ${happy.failed} failed across ${happy.obligations.length} obligation(s).`);
    lines.push('');
    ...
    for (const r of failure.obligations) {
        lines.push(`| ${r.obligationType} | ${r.observedPersona ?? '∅'} | ${r.satisfied ? '✓' : '✗'} | ${truncate(r.detail, 80)} |`);
    }
    lines.push('');
    lines.push(`Run aggregate: ${failure.satisfied} satisfied / ${failure.failed} failed (expected ${failure.failuresExpected} failures across the five Phase 7 types).`);
    lines.push('');
    lines.push('## Phase 7 §10 verdict');
    lines.push('');
    lines.push(`- **At least 7 personas in the library:** ${gates.personaCountAtLeast7 ? 'PASS' : 'FAIL'} (got ${happy.personaCount})`);
    lines.push(`- **At least 8 contract obligation types:** ${gates.obligationTypeCountAtLeast8 ? 'PASS' : 'FAIL'} (got ${happy.obligationTypeCount})`);
    lines.push(`- **Every obligation type dispatches to its owning persona:** ${gates.everyTypeDispatchedToOwner ? 'PASS' : 'FAIL'}`);
    lines.push(`- **Failure suite catches every new Phase 7 obligation type:** ${gates.failureSuiteCatchesEveryNewType ? 'PASS' : 'FAIL'}`);
    lines.push('');
    ...
    lines.push('');
    lines.push('StubSession is deterministic; re-running on the same source tree yields identical numbers.');
    lines.push('');
    return lines.join('\n');
}
...
async function main() {
    ...
    process.stderr.write(`[bench7] personaCount=${happy.personaCount} obligationTypeCount=${happy.obligationTypeCount} happy.failed=${happy.failed} failure.failed=${failure.failed} (expected ${failure.failuresExpected})\n`);
    ...
    process.stderr.write(`[bench7] personas >= 7: ${gates.personaCountAtLeast7 ? 'PASS' : 'FAIL'}\n`);
    process.stderr.write(`[bench7] obligation types >= 8: ${gates.obligationTypeCountAtLeast8 ? 'PASS' : 'FAIL'}\n`);
    process.stderr.write(`[bench7] dispatch correct: ${gates.everyTypeDispatchedToOwner ? 'PASS' : 'FAIL'}\n`);
    process.stderr.write(`[bench7] failure suite catches every new type: ${gates.failureSuiteCatchesEveryNewType ? 'PASS' : 'FAIL'}\n`);
    const ok = gates.personaCountAtLeast7 &&
...
main().catch((err) => {
    process.stderr.write(`[bench7] error: ${err.message}\n`);
    process.exit(1);
});
