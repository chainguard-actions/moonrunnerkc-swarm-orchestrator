...
exports.runTrickyGoal = runTrickyGoal;
...
function makeContract(repoRoot, goal) {
 ...
 */
async function runTrickyGoal(goal, options) {
    const v6Model = options.v6Model ?? v6_model_1.DEFAULT_V6_MODEL;
    const projectContext = options.projectContext ?? run_goal_1.BENCH_PROJECT_CONTEXT;
    ...
    const ledger = new jsonl_ledger_1.JsonlLedger(path.join(work, 'ledger.jsonl'), `${goal.id}-${options.mode}`);
