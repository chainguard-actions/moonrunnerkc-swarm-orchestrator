...
exports.TRICKY_BENCH_GOALS = void 0;
exports.assertTrickyGoalsShape = assertTrickyGoalsShape;
exports.TRICKY_BENCH_GOALS = [
    {
...
function assertTrickyGoalsShape() {
    if (exports.TRICKY_BENCH_GOALS.length < 3) {
        throw new Error(`tricky-goals suite must have at least 3 goals; got ${exports.TRICKY_BENCH_GOALS.length}`);
    }
    for (const g of exports.TRICKY_BENCH_GOALS) {
        if (g.expectedFailureRate < 0.3 || g.expectedFailureRate > 0.7) {
            throw new Error(`tricky goal ${g.id} expectedFailureRate ${g.expectedFailureRate} outside [0.3, 0.7]`);
        }
        const hasMarkerFile = g.obligations.some((o) => o.type === 'file-must-exist' && o.path === g.markerFile);
        if (!hasMarkerFile) {
            throw new Error(`tricky goal ${g.id} markerFile ${g.markerFile} not in obligation list`);
        }
        const hasMarkerCheck = g.obligations.some((o) => o.type === 'build-must-pass' && o.command.includes(g.marker));
        if (!hasMarkerCheck) {
            throw new Error(`tricky goal ${g.id} marker ${g.marker} missing from any build-must-pass command`);
        }
    }
}
