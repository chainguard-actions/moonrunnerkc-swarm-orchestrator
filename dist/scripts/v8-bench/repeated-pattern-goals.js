...
exports.REPEATED_PATTERN_GOALS = void 0;
exports.assertRepeatedPatternGoalsShape = assertRepeatedPatternGoalsShape;
exports.REPEATED_PATTERN_GOALS = [
    {
        id: 'health-checks-3',
...
function assertRepeatedPatternGoalsShape() {
    if (exports.REPEATED_PATTERN_GOALS.length !== 3) {
        throw new Error(`expected 3 repeated-pattern goals, got ${exports.REPEATED_PATTERN_GOALS.length}`);
    }
    for (const g of exports.REPEATED_PATTERN_GOALS) {
        const repeated = g.obligations.filter((o) => o.type === g.repeatedType).length;
        if (repeated !== g.repeatedCount) {
            throw new Error(`repeated-pattern goal ${g.id} declares repeatedCount=${g.repeatedCount} but contract has ${repeated} obligations of type ${g.repeatedType}`);
        }
